...
async function main() {
  await Promise.all([
    ...
    esbuild.build({
      ...SHARED_OPTIONS,
      entryPoints: [path.join(ROOT, 'src/process/resources/builtinMcp/composioToolRouterProxy.ts')],
      outfile: path.join(ROOT, 'out/main/builtin-mcp-composio-tool-router.js'),
    }),
    esbuild.build({
      ...SHARED_OPTIONS,
      entryPoints: [path.join(ROOT, 'src/process/resources/builtinMcp/agentVaultServer.ts')],
      outfile: path.join(ROOT, 'out/main/builtin-mcp-agent-vault.js'),
    }),
    esbuild.build({
      ...SHARED_OPTIONS,
