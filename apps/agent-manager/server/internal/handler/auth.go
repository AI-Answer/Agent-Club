...
func (h *Handler) AgentClubLogin(w http.ResponseWriter, r *http.Request) {
	if os.Getenv("AGENT_CLUB_AUTO_LOGIN") != "1" || strings.EqualFold(os.Getenv("APP_ENV"), "production") {
		writeError(w, http.StatusNotFound, "local Agent Club login is not available")
		return
	}

	email := strings.ToLower(strings.TrimSpace(os.Getenv("AGENT_CLUB_AUTO_LOGIN_EMAIL")))
	if email == "" {
		email = "agent-club@local.agentclub"
	}

	user, _, err := h.findOrCreateUser(r.Context(), email)
	if err != nil {
		slog.Warn("agent club login failed", append(logger.RequestAttrs(r), "error", err, "email", email)...)
		writeError(w, http.StatusInternalServerError, "failed to create local session")
		return
	}

	if updated, err := h.Queries.MarkUserOnboarded(r.Context(), user.ID); err != nil {
		slog.Warn("failed to mark local agent club user onboarded", append(logger.RequestAttrs(r), "user_id", uuidToString(user.ID), "error", err)...)
	} else {
		user = updated
	}
	...
	if err != nil {
		slog.Warn("failed to generate local agent club token", append(logger.RequestAttrs(r), "error", err, "email", email)...)
		writeError(w, http.StatusInternalServerError, "failed to create local session")
		return
	}

	if err := auth.SetAuthCookies(w, token); err != nil {
		slog.Warn("failed to set local agent club auth cookies", "error", err)
	}
	...
	slog.Info("local agent club user logged in", append(logger.RequestAttrs(r), "user_id", uuidToString(user.ID), "email", user.Email)...)
