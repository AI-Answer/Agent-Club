...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, goal_id
`
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, goal_id FROM issue
WHERE id = $1
`
...
WHERE workspace_id = $1
  ...
  AND ($10::uuid IS NULL OR goal_id = $10)
ORDER BY position ASC, created_at DESC
LIMIT $2 OFFSET $3
`
...
WHERE workspace_id = $1
  ...
  AND ($7::uuid IS NULL OR goal_id = $7)
ORDER BY position ASC, created_at DESC
`
