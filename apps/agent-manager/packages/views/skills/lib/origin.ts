 ...
 * Skills.sh, GitHub, JourneyKits). Manual creates have no origin, so we synthesize
 * `{ type: "manual" }` for them to keep the consumer code uniform.
 */
export type OriginInfo = {
  type: "runtime_local" | "clawhub" | "skills_sh" | "github" | "journeykits" | "manual";
  provider?: string;
  runtime_id?: string;
  source_path?: string;
  source_url?: string;
...
export function readOrigin(skill: SkillSummary): OriginInfo {
  ...
  if (raw?.type === "journeykits") return raw;
  return { type: "manual" };
}
