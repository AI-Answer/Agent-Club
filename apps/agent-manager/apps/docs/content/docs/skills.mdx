...
- **From JourneyKits** — paste a `journeykits.ai/kits/{owner}/{slug}` URL, or install public JourneyKits from Agent Club's Skills Hub. Multica downloads the JourneyKits skill archive, keeps the supporting files, and records JourneyKits as the origin.
...
## Publishing to JourneyKits

JourneyKits publishing is intentionally owner-gated. Public discovery and skill-archive installs work without credentials, but submitting, updating, deleting, or releasing a kit requires a JourneyKits account/API key with `kits:write` owner access.

In Agent Club, open **Settings → Skills Hub → JourneyKits Repository** to save that key locally. The key is stored in your local app config, not in the repo or workspace export, and the UI only shows that a key is saved. After the key and owner are set, use **Upload** on any local skill card to publish it as a JourneyKit revision, then use **My JourneyKits** to refresh, open, or delete owned kits.

For teams that manage publisher keys outside Agent Club, keep public discovery/install in the app and run publish operations from the JourneyKits console or API. Agent Club only reports a publish succeeded after the JourneyKits API accepts the upload.
