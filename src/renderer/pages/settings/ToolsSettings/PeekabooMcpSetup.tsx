...
const PEEKABOO_PACKAGE = '@steipete/peekaboo';
...
const buildPeekabooOriginalJson = (): string =>
  JSON.stringify(
    {
      mcpServers: {
        [PEEKABOO_SERVER_NAME]: {
          command: 'npx',
          args: ['-y', PEEKABOO_PACKAGE, 'mcp'],
        },
      },
    },
    null,
  ...
  const handleSave = async () => {
    ...
    try {
      await onSaveServer({
        ...
        description: 'macOS desktop observation and automation MCP for supervised Hermes computer-control sessions.',
        enabled: true,
        transport: {
          type: 'stdio',
          command: 'npx',
        ...
        originalJson: buildPeekabooOriginalJson(),
      });
    } catch (err) {
    ...
    <div className='mb-4 rounded-12px border border-solid border-[var(--border-2)] bg-[var(--fill-1)] p-4'>
      <div className='flex flex-col gap-12px'>
        <div className='flex flex-col gap-10px lg:flex-row lg:items-start lg:justify-between'>
          <div className='min-w-0 flex-1'>
            ...
            <div className='max-w-780px text-xs leading-18px text-t-secondary'>
              Add Peekaboo as a local MCP so Hermes can observe and control the Mac during explicit supervised runs.
              Agent Club will not grant Screen Recording, Accessibility, or app-control permission for you.
            </div>
          </div>
        ...
        <div className='grid grid-cols-1 gap-8px md:grid-cols-3'>
          <PeekabooGate label='Install path' value='npx -y @steipete/peekaboo mcp' tone='blue' />
        ...
        <div className='flex flex-col gap-8px lg:flex-row lg:items-center lg:justify-between'>
          ...
          <Button type='primary' icon={<LinkCloud size={'14'} />} loading={isSaving} onClick={handleSave}>
            {isConfigured ? 'Update Peekaboo MCP' : 'Add Peekaboo MCP'}
          </Button>
        </div>
      </div>
