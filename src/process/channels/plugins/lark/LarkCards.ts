...
export function createMainMenuCard(): LarkCard {
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: 'Agent Club Assistant' },
      template: 'blue',
    },
    elements: [
      {
    ...
    elements: [
      {
        ...
        content: [
          '**What is pairing?**',
          'Pairing links your Lark/Feishu account with the local Agent Club application.',
...
export function createHelpCard(): LarkCard {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '❓ Agent Club Assistant Help' },
      template: 'turquoise',
    },
    elements: [
      {
        tag: 'markdown',
        content: [
          'A remote assistant to interact with Agent Club via Lark.',
          '',
          '**Common Actions:**',
